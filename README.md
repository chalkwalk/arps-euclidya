...
* **32 Global Macros**: A fully visual macro system with color-coded knobs, alt+drag binding, intensity arcs, dual-value display, and bipolar/unipolar toggling — featuring integrated MIDI learn and CC mapping to expose complex internal patch states to DAW automation and hardware controllers.
...
* **Microtonal & Tuning-Aware Quantization**: Load Scala/KBM tuning files and the Quantizer node automatically adapts — its step grid resizes to match the tuning's step count, harmonic-proximity shading highlights consonant intervals, and custom scales can be authored and saved per tuning.
