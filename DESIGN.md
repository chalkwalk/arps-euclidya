...
When a usable host transport is absent — the standalone app, headless unit tests, or transport-less hosts — `ClockManager` runs its own **internal transport** instead. The JUCE standalone wrapper installs a playhead that reports only time-in-samples (no BPM/PPQ, `isPlaying=false`), so a position lacking *both* BPM and PPQ is treated as "no usable host" and routed to the internal branch (belt-and-braces alongside the `useInternalTransport` flag the processor sets for standalone). The internal transport exposes a seekable `transportPpq` (play/pause/seek/reset) plus an optional loop region that wraps `transportPpq` at the loop end. By reproducing the exact PPQ/`isPlaying` signals a DAW emits — including the backward jump a loop wrap or a jog produces — it drives `MidiOutNode`'s existing tick-detection and backward-jump logic unchanged; `MidiOutNode` is unaware it isn't being driven by a host. The standalone UI for this transport lives in `TransportBar` (`src/TransportBar.h/cpp`) and `TimelineRuler` (`src/TimelineRuler.h/cpp`): a bar.beat.tick readout, a play/reset/loop/BPM bar, and a jog-able mini-timeline with a loop region, zoom/scroll, manual pan (middle-button drag or Shift+wheel), and auto-follow that keeps the playhead within the central band of the view while playing (capped so it rides the far left at the timeline start).
