...
document.getElementById('node-w').addEventListener('change', (e) => {
    if (currentData) { currentData.gridWidth = parseInt(e.target.value); renderAll(); }
});
document.getElementById('node-h').addEventListener('change', (e) => {
    if (currentData) { currentData.gridHeight = parseInt(e.target.value); renderAll(); }
});
...
function renderAll() {
    ...
    if (selectedElementIndex >= 0 && selectedElementIndex < currentData.elements.length) {
...
function drawToggle(ctx, el, r) {
    // Mirrors TextButton (getForegroundSlate background, white text, neon border on hover)
    const R = 4 * SCALE / 3;
